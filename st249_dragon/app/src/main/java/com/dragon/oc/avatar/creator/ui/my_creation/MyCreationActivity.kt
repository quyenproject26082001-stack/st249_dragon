package com.dragon.oc.avatar.creator.ui.my_creation
...
import com.dragon.oc.avatar.creator.core.custom.text.DoubleStrokeTextView
import com.dragon.oc.avatar.creator.core.custom.text.OuterStrokeTextView
...
import com.dragon.oc.avatar.creator.core.utils.share.whatsapp.WhatsappSharingActivity
import com.dragon.oc.avatar.creator.core.utils.state.HandleState
import com.dragon.oc.avatar.creator.databinding.ActivityAlbumBinding
import com.dragon.oc.avatar.creator.dialog.YesNoDialog
import com.dragon.oc.avatar.creator.ui.home.HomeActivity
import com.dragon.oc.avatar.creator.ui.view.ViewActivity
import com.dragon.oc.avatar.creator.dialog.CreateNameDialog
import com.dragon.oc.avatar.creator.ui.my_creation.adapter.MyAvatarAdapter
import com.dragon.oc.avatar.creator.ui.my_creation.adapter.TypeAdapter
import com.dragon.oc.avatar.creator.ui.my_creation.fragment.MyAvatarFragment
import com.dragon.oc.avatar.creator.ui.my_creation.fragment.MyDesignFragment
import com.dragon.oc.avatar.creator.ui.my_creation.view_model.MyAvatarViewModel
import com.dragon.oc.avatar.creator.ui.my_creation.view_model.MyCreationViewModel
...
class MyCreationActivity : WhatsappSharingActivity<ActivityAlbumBinding>() {
        ...
        binding.apply {
            lifecycleScope.launch {
                repeatOnLifecycle(Lifecycle.State.CREATED) {
                    ...
                    launch {
                        viewModel.hasAvatarItems.collect {
                            updateBottomButtonsVisibility()
                        }
                    }
                    launch {
                        viewModel.downloadState.collect { state ->
                            when (state) {
                                ...
                                HandleState.SUCCESS -> {
                                    ...
                                    resetCurrentSelectionModeIfNeeded()
                                }
    ...
    override fun viewListener() {
        binding.apply {
            ...
            layoutBottom.findViewById<View>(R.id.btnWhatsapp)?.tap(2500) {
                val paths = getSelectedOrAllAvatarPaths()
            ...
            layoutBottom.findViewById<View>(R.id.btnShare)?.tap(2500){
    ...
    private fun getSelectedOrAllAvatarPaths(): ArrayList<String> {
        val avatarFragment = supportFragmentManager.findFragmentByTag("MyAvatarFragment")
        val selectedPaths = getSelectedPathsFromCurrentFragment()
        if (selectedPaths.isNotEmpty()) return selectedPaths
        if (avatarFragment !is MyAvatarFragment || !avatarFragment.isVisible) return selectedPaths
...
//        binding.actionBar.btnActionBarRight.setImageResource(R.drawable.ic_select_all)
        ...
        return avatarFragment.getAllPaths()
    }
    ...
    override fun initActionBar() {
        binding.actionBar.apply {
            ...
            btnActionBarNextRight.setImageResource(R.drawable.ic_delete_item_mywork)
            btnActionBarNextRight.gone()
        }
    }
    ...
    private fun resetCurrentSelectionModeIfNeeded() {
        val avatarFragment = supportFragmentManager.findFragmentByTag("MyAvatarFragment")
        val designFragment = supportFragmentManager.findFragmentByTag("MyDesignFragment")
        when {
            avatarFragment is MyAvatarFragment && avatarFragment.isVisible -> avatarFragment.resetSelectionMode()
...
//    }
    ...
    private fun updateBottomButtonsVisibility() {
        val bottom = binding.includeLayoutBottom
        val btnWhatsapp = bottom.btnWhatsapp
        val btnTelegram = bottom.btnTelegram
        val btnDownload = bottom.btnDownload
        val btnShare = bottom.btnShare
        val isAvatarTab = viewModel.typeStatus.value == ValueKey.AVATAR_TYPE
        val hasAvatarItems = viewModel.hasAvatarItems.value

        if (isInSelectionMode || (isAvatarTab && hasAvatarItems)) {
            binding.lnlBottom.visible()
        } else {
            binding.lnlBottom.gone()
        }

        if (isAvatarTab && hasAvatarItems) {
            btnWhatsapp?.visible()
            btnTelegram?.visible()
        } else {
            btnWhatsapp?.gone()
