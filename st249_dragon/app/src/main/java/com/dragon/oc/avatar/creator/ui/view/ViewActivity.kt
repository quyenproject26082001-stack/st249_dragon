package com.dragon.oc.avatar.creator.ui.view
...
import com.dragon.oc.avatar.creator.databinding.ActivityViewBinding
...
class ViewActivity : BaseActivity<ActivityViewBinding>() {
    ...
    private fun resolveDisplayPath() {
        ...
        val dragonCards = MediaHelper
            .readListFromFile<DragonCardEditModel>(this, ValueKey.DRAGON_CARD_EDIT_FILE_INTERNAL)
        val dragonCard = when {
            viewModel.dragonModelId.isNotEmpty() -> {
                dragonCards.firstOrNull { it.id == viewModel.dragonModelId }
            }
            else -> {
                dragonCards.firstOrNull { it.previewPath == viewModel.pathInternal.value }
            }
        }

        if (dragonCard != null) {
            viewModel.dragonModelId = dragonCard.id
            viewModel.setPathInternal(dragonCard.previewPath)
        }
        ...
        } else if (File(viewModel.pathInternal.value).exists()) {
            viewModel.setDisplayPath(viewModel.pathInternal.value)
        }
    }
    ...
    private fun resetMyCreationSelectionMode() {
        ...
        if (myCreationActivity != null) {
            ...
            if (designFragment is com.dragon.oc.avatar.creator.ui.my_creation.fragment.MyDesignFragment) {
            ...
            withContext(Dispatchers.Main) {
                val intent = if (!dragonCard?.selectionState.isNullOrBlank()) {
                    Intent(this@ViewActivity, DragonWebViewActivity::class.java).apply {
                        ...
                        putExtra(DragonWebViewActivity.EXTRA_EDIT_MODEL_ID, dragonCard?.id)
                    ...
                    Intent(this@ViewActivity, EditActivity::class.java).apply {
                        ...
                        putExtra(IntentKey.EDIT_SOURCE_PATH, pathInternal)
                        if (dragonCard?.id?.isNotEmpty() == true) {
                            putExtra(IntentKey.EDIT_MODEL_ID, dragonCard.id)
                        }
                    }
                }
